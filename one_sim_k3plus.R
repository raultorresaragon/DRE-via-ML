...
# PARAMERTERS FOR DEBUGGING
   ...
   ### Y_flavor = "gamma"
   ### beta_Y_scalar = 1
   ### gamma <- c(1.1, 2.2, 3, -1.1)[1:(k-1)] * 0 #c(0.8, 0.6, 0.52, 0.37)
...
one_sim <- function(n, p, Xmu, beta_A, beta_Y, gamma, k,
  ...
  dat <- cbind(Y,A,X) 
  stopifnot(Y>=0)
  #readr::write_csv(dat, paste0("datasets/df_k", k, A_flavor,Y_flavor,"_dset",iter,".csv"))
  ...
  #i=0
  #plot(sort(Y[A==i]), main="sorted", cex.main=2, col=mycols[i+1], xlim=c(0,n))
  #for(i in 1:(k-1)){
  ...
  logit_expo_est <- sapply(fit_Y_ols, function(x) {x[[1]]}) |> data.frame()
