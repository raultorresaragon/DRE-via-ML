...
def save_boxplots(filepath, save=True, show=True):
    ...
    print(f"Parsed: k={k}, A_flavor={A_flavor}, Y_flavor={Y_flavor}")
    ...
    fig, axes = plt.subplots(1, n_comparisons,
                             figsize=(4 * n_comparisons, 5), squeeze=False)
    axes = axes.flatten()
    ...
    fig.suptitle(f'Estimation Errors: DGP = {A_flavor}-{Y_flavor}, |A|={k}',
...
def save_pooled_boxplot(filepath, save=True, show=True):
    """
    Create a single pooled boxplot (all comparisons combined).
    Models shown are detected dynamically from the CSV estimate column.
    ...
    box_data = [plot_df[plot_df['model'] == m]['error'].values for m in model_order]
    box_colors = [colors.get(m, 'gray') for m in model_order]
    ...
    if save:
        images_dir = os.path.dirname(filepath).replace('/tables', '/images')
        os.makedirs(images_dir, exist_ok=True)
        out_path = os.path.join(images_dir, f"boxplot_pooled_k{k}_{A_flavor}_{Y_flavor}.png")
        plt.savefig(out_path, dpi=150, bbox_inches='tight')
        print(f"Saved: {out_path}")
