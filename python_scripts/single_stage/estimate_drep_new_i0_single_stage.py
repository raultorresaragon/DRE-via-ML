# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# estimate_drep_new_i0_single_stage.py
...
#
# Output per (k, flavor):
#   models saved : datasets/new_i0/models/s1_k{k}_simple_{flavor}_0_DREp_models.pkl
#   predictions  : datasets/new_i0/s1_k{k}_simple_{flavor}_new_i0_DREp.csv
#     columns: d_star_1, Y_hat_1_a0, ..., Y_hat_1_a{k-1}
#
# V(DRE-Param) = mean(Y_hat_1[i, d_star_1[i]])  — evaluated by vplot_new_i0_stage1.py
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def estimate_drep_new_i0(k, flavor_Y, i_source=0):
    """Fit DRE-Param on old i=i_source (single-stage); apply outcome models to new_i0."""
    ...
    print(f"\n{'='*60}")
    print(f"DRE-Param new_i0 (single-stage): k={k}, flavor={flavor_Y}")
    ...
    for a in range(k):
        mask    = (A1_i0 == a)
        model_a = LinearRegression().fit(X1_i0[mask], Y_i0[mask])
        models_Y1[a]        = model_a
        Y1_hat_all_i0[:, a] = model_a.predict(X1_i0)
        print(f"    OLS Y a={a}  (n={mask.sum()})")
