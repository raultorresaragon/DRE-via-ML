...
def get_type1error(k, A_flavor, Y_flavor, Y_param, zero_effect):
    for modtype in ['nn','param']:
        root = f"./_{'1' if not zero_effect else '0'}trt_effect"
        csv_path = f"{root}/tables/muhat_pooled_simk{k}_{A_flavor}_{Y_flavor}_est_with_{Y_param}_" + modtype + ".csv"
        ...
        output_path = f"{root}/tables/type_1_error_simk{k}_{A_flavor}_{Y_flavor}_est_with_{Y_param}_" + modtype +".csv"
