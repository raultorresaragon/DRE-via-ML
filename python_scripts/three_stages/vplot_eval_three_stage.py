...
#           _1trt_effect/3stages/tables/eval_sets/_v_per_replication.csv
#
...
def make_figure(sub_df, k, flavor, greyscale=False, include_drep_ols=True):
    ...
    ax.set_title(f'V(d) for three stage  |A|={k}  ({title_flavor})', fontsize=11)
    ax.set_xticks(xs)
...
if __name__ == '__main__':
    ...
    if df_all.empty:
        print('No data found. Run gen_eval_datasets_three_stage.py, '
              'run_eval_dre_three_stage.py, and run_eval_drep_three_stage.py first.')
    else:
        summary_rows = []
