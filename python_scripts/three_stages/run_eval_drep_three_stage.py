...
# Load DRE-Param (OLS) pkl, apply outcome models to eval data, save predictions.
#
# This script does NOT re-train models. It loads the pkl produced by
# estimate_drep_three_stage.py and applies the stored OLS outcome models to the
...
# Output: eval_sets/{filename}_eval_DREp.csv
...
def run_eval_drep(filename):
    """
    Load the DREp pkl for `filename`, apply to eval data, save _eval_DREp.csv.
    ...
    out_path  = os.path.join(eval_dir,   f'{filename}_eval_DREp.csv')
    ...
    print(f"\n{'='*60}")
    print(f"run_eval_drep (OLS): {filename}")
    print(f"{'='*60}")
    ...
    # feat_2_a: numpy [X1_ev, Y1_ev - Y1_hat_ev[:, a]]
    ...
    # feat_3_a: numpy [X1_ev, Y1_ev - Y1_hat_ev[:, a], Y2_ev - Y2_hat_ev[:, a]]
    ...
    print(f"  ✓ Predictions saved: {filename}_eval_DREp.csv")
    return V
