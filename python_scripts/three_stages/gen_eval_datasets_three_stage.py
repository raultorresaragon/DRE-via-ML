# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# gen_eval_datasets_three_stage.py
# Generate one evaluation dataset per row of _info_simple.csv for the three-stage DGP.
#
...
#
# Saved to: _1trt_effect/3stages/datasets/eval_sets/{filename}_eval.csv
# Info file: _1trt_effect/3stages/datasets/eval_sets/_info_eval.csv
#
...
from YAX_funs   import gen_X, gen_A, gen_A2_simple, gen_A3_simple, gen_X2, \
                       gen_Y2_simple, gen_Y_final_simple
...
def gen_eval_dataset(row, data_seed_offset=DATA_SEED_OFFSET):
    ...
    info_row = pd.DataFrame([{
        ...
        'Delta1':      str([round(float(x), 2) for x in Delta1]),
        'delta2':      str([round(float(delta2[0]), 2)]),
        'Delta2':      str([round(float(Delta2[0]), 2)]),
        'delta3':      str([round(float(delta3[0]), 2)]),
        'Delta3':      str([round(float(Delta3[0]), 2)]),
    }])
    ...
    print(f"  i={i_val}  k={k}  flavor={flavor_Y}  "
          f"A1={np.bincount(A1)}  A2={np.bincount(A2)}  A3={np.bincount(A3)}  "
          f"Y_mean={Y.mean():.3f}")
