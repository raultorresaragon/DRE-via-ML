# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# estimate_naive_new_i0_three_stage.py
# Naive estimator fitted directly on the new_i0 dataset (three-stage).
#
# Decisions : d_star_1, d_star_2, d_star_3 drawn uniformly at random.
# Y_hat     : observed arm mean for Y1, Y2, Y — broadcast to all n.
#
# Output: datasets/new_i0/s3_k{k}_simple_{flavor}_new_i0_NAIVE.csv
...
def estimate_naive_new_i0(k, flavor_Y):
    ...
    for a in range(k2):
        mask = (A2 == a)
        Y2_hat_all[:, a] = Y2[mask].mean() if mask.sum() > 0 else 0.0

    Y3_hat_all = np.zeros((n, k3))
    for a in range(k3):
        mask = (A3 == a)
        Y3_hat_all[:, a] = Y[mask].mean() if mask.sum() > 0 else 0.0

    V = float(np.mean(Y3_hat_all[np.arange(n), d_star_3]))
    ...
    for a in range(k3):
        out[f'Y_hat_3_a{a}'] = Y3_hat_all[:, a]
