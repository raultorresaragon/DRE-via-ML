# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# estimate_naive_two_stage.py
# Naive baseline estimator for two-stage DTR.
#
# Decisions: d_star_1, d_star_2 drawn uniformly at random for each individual.
# mu_hat   : unadjusted observed arm means (constant per arm, broadcast to all n).
#
# Output format mirrors estimate_dre_two_stage.py so that OTR_assess.py and
# ate_two_stage_simple.py can consume either file unchanged.
...
def estimate_naive(filename, dgp='simple'):
    """
    Naive two-stage estimator: random decisions, unadjusted mu_hat.
    ...
    for a in range(k2):
        mask = (A2 == a)
        mu_hat_2[:, a] = Y[mask].mean() if mask.sum() > 0 else 0.0
