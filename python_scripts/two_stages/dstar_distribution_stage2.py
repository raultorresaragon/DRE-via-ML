# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# dstar_distribution_stage2.py
# Treatment allocation distribution table for a chosen eval dataset.
#
# Reads from eval_sets/:
#   {filename}_eval.csv             — observed (A1, A2)
#   {filename}_eval_DRE.csv         — DRE-ML d_star_1, d_star_2
#   {filename}_eval_DREp_expo.csv   — DRE-Param (expo) d_star_1, d_star_2  [optional]
#   {filename}_eval_DREp_ols.csv    — DRE-Param (ols)  d_star_1, d_star_2  [optional]
#
# Output columns:
...
#
# Control in __main__:
#   FILENAME  — base training filename (e.g. 's2_k2_simple_expo_0'); '_eval' appended auto.
#               Set to None to run over all rows in _info_simple.csv (use K/FLAVOR filters).
#   K_FILTER, FLAVOR_FILTER — used only when FILENAME is None.
#
# Output: _1trt_effect/2stages/tables/eval_sets/_dstar_dist_{filename}_eval.csv
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
tables_dir   = os.path.join(script_dir,   '../_1trt_effect/2stages/tables/eval_sets')
...
def make_dist_table(filename, include_drep_expo=True, include_drep_ols=True):
    ...
    Parameters
    ----------
    filename         : str   Base training filename (e.g. 's2_k2_simple_expo_0').
    ...
    # Infer k from observed treatment levels
    ...
    for a in arms:
        ...
        if has_expo:
            row['d_star_DREp-expo'] = round((df_expo['d_star_1'] == a).sum() / n, 4)
        if has_ols:
    ...
    for a1, a2 in product(arms, arms):
        ...
        if has_expo:
            expo_mask = (df_expo['d_star_1'] == a1) & (df_expo['d_star_2'] == a2)
...
if __name__ == '__main__':
    ...
    FILENAME      = 's2_k2_simple_gamma_0'   # e.g. 's2_k3_simple_gamma_5' or None
