# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# estimate_drep_new_i0_two_stage.py
...
#
# Output per (k, flavor):
#   models saved : datasets/new_i0/models/s2_k{k}_simple_{flavor}_0_DREp_models.pkl
#   predictions  : datasets/new_i0/s2_k{k}_simple_{flavor}_new_i0_DREp.csv
...
def estimate_drep_new_i0(k, flavor_Y):
    ...
    print(f"\n{'='*60}")
    print(f"DRE-Param new_i0 (2-stage): k={k}, flavor={flavor_Y}")
    ...
    # ----------------------------------------------------------------
    # Stage 1 — fit OLS outcome models on i=0
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # Stage 2 — fit OLS outcome models on i=0
    # ----------------------------------------------------------------
    ...
    for a in range(k2):
        ...
        feat_2_a = np.hstack([X1_i0, X2_i0, resid_a])
        mask     = (A2_i0 == a)
        model_a  = LinearRegression().fit(feat_2_a[mask], Y_i0[mask])
        models_Y2[a]        = model_a
    ...
    for a in range(k2):
        ...
        feat_2_a = np.hstack([X1_new, X2_new, resid_a])
