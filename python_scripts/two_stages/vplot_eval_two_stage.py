...
#   Bar plot: _1trt_effect/2stages/images/eval_sets/_vplot_eval_k{k}_{flavor}[_bw].jpeg
#     4 bars: DRE-ML | DREp-expo | DREp-ols | Obs Y
#     Each bar top shows mean V (3 d.p.) + significance stars vs DRE-ML:
#       *** p < 0.01 | ** p < 0.05 | * p < 0.10  (DRE-ML and Obs Y show no stars)
#   Tables: _1trt_effect/2stages/tables/eval_sets/_v_summary.csv
#           _1trt_effect/2stages/tables/eval_sets/_v_per_replication.csv
#
...
images_dir   = os.path.join(script_dir,   '../_1trt_effect/2stages/images/eval_sets')
...
def make_figure(sub_df, k, flavor, greyscale=False):
    ...
    # Compute paired t-test p-values (DRE-ML as reference)
    pval_expo = np.nan
    pval_ols  = np.nan
    ...
    offset  = y_range * 0.03 + abs(max(means)) * 0.01   # small gap above bar
    ...
    ax.set_title(f'V(d) for two stage  |A|={k}  ({title_flavor})', fontsize=11)
    ax.set_xticks(xs)
    ...
    if df_all.empty:
        ...
        for (k, flavor), sub in df_all.groupby(['k', 'flavor_Y']):
            ...
            if len(v_dre) > 0 and len(v_drep_expo) > 0 and len(v_dre) == len(v_drep_expo):
                ...
                row['sig_vs_expo']   = _stars(p) or 'n.s.'   # *** / ** / * / n.s.
            else:
            ...
            if len(v_dre) > 0 and len(v_drep_ols) > 0 and len(v_dre) == len(v_drep_ols):
                ...
                row['sig_vs_ols']   = _stars(p) or 'n.s.'    # *** / ** / * / n.s.
            else:
